...
# Save reference to real time.sleep before any patching
...
class TestZoomWebBot(TransactionTestCase):
    ...
    @patch("bots.zoom_web_bot_adapter.zoom_web_ui_methods.time.sleep", side_effect=lambda x: _real_sleep(1))
    @patch("bots.zoom_web_bot_adapter.zoom_web_ui_methods.start_zoom_web_static_server", return_value=8080)
    @patch("bots.web_bot_adapter.web_bot_adapter.Display")
    @patch("bots.web_bot_adapter.web_bot_adapter.webdriver.Chrome")
    @patch("bots.bot_controller.bot_controller.S3FileUploader")
    def test_generic_join_error_retries_then_stops(
        ...
        """
        Test that when a generic join error occurs, the bot retries until the timeout is exceeded,
        then stops with a COULD_NOT_JOIN event.
        """
        # Configure the mock uploader
        mock_uploader = create_mock_file_uploader()
        MockFileUploader.return_value = mock_uploader
        ...
        # Track how many times the generic join error check is called
        generic_join_error_call_count = [0]
        ...
        def execute_script_side_effect(script, *args):
            ...
            if "userHasEncounteredGenericJoinError" in script:
                generic_join_error_call_count[0] += 1
                return True  # Always return generic join error
            return None
        ...
        # Lower the generic_join_error_retry_timeout_seconds to make the test faster
        # We do this after the controller creates the adapter
        short_timeout = 3  # Set a very short timeout (2 seconds)
        ...
        # Override the timeout on the adapter
        if hasattr(controller, "adapter") and controller.adapter:
            controller.adapter.generic_join_error_retry_timeout_seconds = short_timeout

        # Give the bot time to process - should retry a few times then timeout
        bot_thread.join(timeout=10)
        ...
        # Verify that the generic join error check was called multiple times (indicating retries)
        self.assertGreater(generic_join_error_call_count[0], 1, "Generic join error should be checked multiple times (retries)")
        ...
        # Find the COULD_NOT_JOIN event
        could_not_join_events = [e for e in bot_events if e.event_type == BotEventTypes.COULD_NOT_JOIN]
        self.assertGreaterEqual(len(could_not_join_events), 1, "Expected at least one COULD_NOT_JOIN event")
        ...
        # Verify the event metadata contains the error code
        self.assertIn("zoom_result_code", could_not_join_event.metadata)
        self.assertIn("Fail to join the meeting", could_not_join_event.metadata["zoom_result_code"])
