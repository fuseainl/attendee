...
| Variable | Type | Default | Description |
|----------|------|---------|-------------|
| (None currently documented) | - | - | Feature flags for enabling/disabling specific functionality are managed through the settings and code. |

---

## Generating Initial Environment Variables

To generate a base `.env` file with required variables, run:
...
This generates:
- `CREDENTIALS_ENCRYPTION_KEY` - Random Fernet encryption key
- `DJANGO_SECRET_KEY` - Random Django secret key  
- `AWS_RECORDING_STORAGE_BUCKET_NAME` - Empty (requires manual entry)
- `AWS_ACCESS_KEY_ID` - Empty (requires manual entry)
- `AWS_SECRET_ACCESS_KEY` - Empty (requires manual entry)
- `AWS_DEFAULT_REGION` - Defaults to `us-east-1`

After running this command, edit the `.env` file and add your specific configuration values.
...
- `DISABLE_EMAIL=true` (emails printed to console)
- `POSTGRES_SSL_REQUIRE=false`
- `REQUIRE_HTTPS_WEBHOOKS=false`

### Production Environment (`production.py`)
- `DJANGO_SSL_REQUIRE=true` (recommended)
- `DISABLE_EMAIL=false`
- `POSTGRES_SSL_REQUIRE=true`
- `REQUIRE_HTTPS_WEBHOOKS=true` (recommended)
- All secrets must be set (AWS/Azure credentials, email credentials, etc.)
...
2. **Rotate secrets regularly** - Especially `DJANGO_SECRET_KEY` and `CREDENTIALS_ENCRYPTION_KEY`
3. **Use vault systems** - For production, use AWS Secrets Manager, Azure Key Vault, or similar
4. **Enable SSL everywhere** - Keep `DJANGO_SSL_REQUIRE=true` and `POSTGRES_SSL_REQUIRE=true` in production
5. **Monitor Sentry** - Enable Sentry in production to catch errors early
6. **Restrict webhook URLs** - Use `REQUIRE_HTTPS_WEBHOOKS=true` in production
7. **Mask sensitive data** - Use `MASK_TRANSCRIPT_IN_LOGS=true` if handling sensitive content
...
- For SSL, use `rediss://` scheme and set `REDIS_SSL_REQUIREMENTS` appropriately
- Check firewall and network connectivity

### Storage Configuration Issues
- For AWS: Ensure IAM credentials have S3 access and bucket exists in specified region
- For Azure: Verify connection string and container exist
- Ensure buckets/containers have proper CORS settings if serving files directly

### Email Not Working
- Verify SMTP credentials are correct
- Check firewall allows outbound SMTP (port 587)
- Review logs for specific error messages
- Use `DISABLE_EMAIL=true` during development

---

## Related Documentation

- [AWS S3 Setup Guide](./aws_setup.md)
- [Azure Storage Setup Guide](./azure_setup.md)
- [Bot Configuration](./voice_agents.md)
